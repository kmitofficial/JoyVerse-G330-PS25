...
IMAGES_DIR = "captured_images"  # Folder to store images
CSV_FILE = "landmarks.csv"       # File to store landmarks
SVM_MODEL_FILE = "svm_model.pkl" # File to save/load the trained SVM model
...
def compute_head_movement(landmarks):
    """
    Determines head movement direction based on key face landmarks.
    ...
    try:
        ...
        right_eye_x = landmarks[263 * 3]
        nose_x = landmarks[1 * 3]
        nose_y = landmarks[1 * 3 + 1]  # Nose Y coordinate
    ...
    threshold_x = 0.015  # Sensitivity for Left/Right
    threshold_y = 0.015  # Sensitivity for Up/Down
    ...
    if nose_y < 0.45:  # Nose moves up when y decreases
        movement += " & Up" if movement != "Center" else "Up"
    elif nose_y > 0.55:  # Nose moves down when y increases
        movement += " & Down" if movement != "Center" else "Down"
...
def capture_and_save_landmarks():
    """
    Captures webcam feed, extracts facial landmarks, detects head movement,
    predicts emotions (if trained model exists), and saves data every 3 seconds.
    """
    cap = cv2.VideoCapture(0)
    start_time = time.time()
    frame_count = 0
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                if os.path.exists(SVM_MODEL_FILE):
                    ...
                    cv2.putText(frame, score_str, (30, 60),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    label = emotion  # Use predicted emotion as label
                else:
                    label = "unknown"

                # Save data every 3 seconds
                current_time = time.time()
                if current_time - start_time >= 3:
                    start_time = current_time
                    frame_count += 1
                    ...
                    df_row = pd.DataFrame([row])
                    df_row.to_csv(CSV_FILE, mode='a', header=False, index=False)
                    print(f"Saved frame {frame_count} with label: {label}")
                break  # Process only the first detected face

        cv2.imshow("FaceMesh Emotion Recognition", frame)
        if cv2.waitKey(25) & 0xFF == ord('q'):
            break
...
def train_svm_model():
    """
    Trains an SVM classifier on the saved facial landmarks dataset.
    Requires a labeled dataset with at least two different emotions.
    """
    df = pd.read_csv(CSV_FILE)
    if 'label' not in df.columns or len(np.unique(df['label'])) < 2:
        print("Error: The number of classes must be greater than one. Please label your dataset.")
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FaceMesh SVM Emotion Recognition")
    parser.add_argument('--mode', type=str, default='capture', choices=['capture', 'train'],
                        help="'capture' to capture landmarks, 'train' to train SVM.")
    args = parser.parse_args()
