...
const Game = () => {
  ...
  const adjustDifficulty = async (emotion: string) => {
    ...
    const switchThemeEmotions = ['sad', 'angry', 'bored', 'frustrated'];
    if (switchThemeEmotions.includes(emotion)) {
  ...
  const handleNextLevel = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Fetched emotion:', emotion);
      }
    } catch (error) {
      console.error('Failed to fetch emotion:', error);
    }
    ...
    await adjustDifficulty(emotion);
